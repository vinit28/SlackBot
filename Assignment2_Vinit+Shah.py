...
def getData(topic):
    ...
    data = rows
    result = [ {"Title": row["title"], "Description": row["Description"],"URL": row["url"],"Shares": row["Shares"]} for row in rows if topic.lower() in row["title"].lower() or topic.lower() in row["Description"].lower() or topic.lower() in row["section"].lower()
             ]
    return result
...
def create_message(username, topic):
    ...
    if topic != None:
        ...
        message = "_Thank you @{u} for abusing me. Here is a list of relevant articles on {s}. You will be charged a $5 fee. Please check your bank account now._\n\n".format(u=username, s=topic)
...
#       Add the information for each station
        for articles in matching:
            ...
            message += "*Title*: {a}. \n *Description:* {f}.\n *Link:* {c} \n *Shares:* {e}\n\n".format(a=Title, f=Description, c=Link, e=numShares)
    else:
...
if sc.rtm_connect():
    ...
    while True:
        ...
        for item in response:
            ...
            sc.api_call("chat.postMessage", channel="#assignment2_bots", text=message, unfurl_links=True)
            sc.api_call("chat.postMessage", channel="#vinit", text=message, unfurl_links=True)
